...
def main():
    ...
    author="David D Lowe",
    license="GPL v2",
    data_files=[('/usr/share/dbus-1/system-services/', ['org.nfsmanager.service']),
                ('/usr/share/polkit-1/actions', ['org.nfsmanager.policy']),
                ('/etc/dbus-1/system.d/', ['org.nfsmanager.conf'])],
    scripts=['consumeservice.py','myservice.py'],
...
class install_dbus_service(Command):
    ...
    def run(self):
        ...
        output = ""
        ff = open("/usr/share/dbus-1/system-services/org.nfsmanager.service","r")
        for line in ff.readlines():
            if line.strip()[:5] == "Exec=":
                line = line.replace("/usr/bin", script_install_dir)
            output += line
        ff.close()
        ff = open("/usr/share/dbus-1/system-services/org.nfsmanager.service","w")
        ff.write(output)
        ff.close()
